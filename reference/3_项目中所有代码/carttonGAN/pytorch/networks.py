...
class resnet_block(nn.Module):
    ...
    def forward(self, inputs):
        x = F.relu(self.conv1_norm(self.conv1(inputs)), True)
        x = self.conv2_norm(self.conv2(x))

        return inputs + x  # Elementwise Sum
...
class VGG19(nn.Module):
    def __init__(
        ...
        self.cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 256,
                    'M', 512, 512, 512, 512, 'M', 512, 512, 512, 512, 'M']
        self.init_weights = init_weights
        self.feature_mode = feature_mode
        self.batch_norm = batch_norm
        self.num_clases = num_classes
        self.features = make_layers(self.cfg, batch_norm)
